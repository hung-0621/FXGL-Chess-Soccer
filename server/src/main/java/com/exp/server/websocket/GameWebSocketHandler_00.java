...
@Component
public class GameWebSocketHandler_00 extends TextWebSocketHandler {
        ...
        if (session1 != null && session1.isOpen()) {
            executor.submit(() -> {
                try {
                    ...
                    System.out.println("broadcast 傳送給 player1 失敗：" + e.getMessage());
                }
            });
        }
        ...
        if (session2 != null && session2.isOpen()) {
            executor.submit(() -> {
                try {
                    ...
                    System.out.println("broadcast 傳送給 player2 失敗：" + e.getMessage());
                }
            });
        }
    }
    ...
    public void checkTimeouts() {
        ...
        for (Map.Entry<String, LocalDateTime> entry : GameWebSocketHandler_00.disconnectTimeMap.entrySet()) {
            ...
            if (Duration.between(disconnectTime, now).getSeconds() >= 60) {
                ...
                GameWebSocketHandler_00.sendToToken(match.getPlayer1Id(), msg);
                GameWebSocketHandler_00.sendToToken(match.getPlayer2Id(), msg);

                System.out.println("玩家斷線超過 60 秒，自動判定敗方為：" + token);

                GameWebSocketHandler_00.disconnectTimeMap.remove(token);
            }
            }
        }
    }
            ...
            try {
                ...
                if ("shot".equals(type)) {
                    ...
                    try {
                        ...
                        sendToToken(senderToken, "{\"type\":\"error\",\"message\":\"MoveCommand 解析失敗\"}");
                        return;
                    }
